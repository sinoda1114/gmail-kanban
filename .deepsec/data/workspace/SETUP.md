# Agent setup for `workspace`

This is a deepsec scanning workspace. Project `workspace` was just registered
(target: `..`). Setup is incomplete — `data/workspace/INFO.md`
...
2. **Fill in `data/workspace/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id workspace
pnpm deepsec process --project-id workspace
```
