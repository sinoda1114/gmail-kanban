...
- Cloud Agent ではシークレットを `/home/ubuntu/.config/gmail-kanban-secrets/` に置き、`source .../load.sh`（`~/.bashrc` からも自動読み込み）で注入する。
  ...
  - Gemini: `.env.gemini`（`GOOGLE_GENERATIVE_AI_API_KEY`）。案件整理などの安い JSON は `gemini-3.5-flash-lite`。面談対策（検索グラウンディング）とテキスト通し練習の相手役は `gemini-3.8-flash`。ライブ面談は Gemini Live API（`gemini-3.8-live`）。
