...
| 案件詳細・面談対策 | `getByRole("tab", { name: "面談対策" })`、ボタン `AIで面談対策を作成` |
| 案件詳細・面談準備 | `getByRole("tab", { name: "面談準備" })`、見出し `面談情報` |
| 面談準備の生成 | ボタン `AIで面談準備を一括作成` / `AIで面談準備を再生成（拡張含む）` |
| 振り返り反映の案内 | テキスト `この案件の面談振り返りを、準備の生成に反映します` |
| 対策パック反映の案内 | テキスト `この案件の面談対策パックを、準備の生成に反映します` |
| 案件詳細・面談練習 | `getByRole("tab", { name: "面談練習" })`、ボタン `通し練習を開始` |
