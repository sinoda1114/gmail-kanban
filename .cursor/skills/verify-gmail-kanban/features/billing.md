...
- **Open billing.** Choose `getByRole("link", { name: "プラン" })`. URL is `/dashboard/billing`. Status is 200. Heading `プラン` is visible. If the document is a Next error overlay (`This page couldn’t load` / status 500), stop.
