...
- 呼び出し側（`.github/workflows/ci.yml`）は Clerk 用 Secrets を **明示マップ**する（`secrets: inherit` は使わない）
- `ci-standard` の e2e ジョブが optional Secrets を env に渡すこと（[ci-standard#2](https://github.com/sinoda1114/ci-standard/issues/2)）。適用パッチは `notes/patches/`
